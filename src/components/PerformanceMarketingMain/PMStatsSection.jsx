...
const PMStatsSection = () => {
  const computedColorScheme = useComputedColorScheme('light');

  const stats = [
    { number: '4.5x', label: 'Average ROAS Delivered' },
    { number: '₹50Cr+', label: 'Ad Spend Managed' },
    { number: '65%', label: 'Avg. CPA Reduction' },
    ...
    >
      <Container size="lg">
        <Box ta="center" mb={60}>
          ...
          >
            Numbers That{' '}
            <Text component="span" c="#14b8a6" inherit>
              Prove It
            </Text>
          </Text>
        </Box>

        <SimpleGrid cols={{ base: 1, sm: 2, md: 4 }} spacing={{ base: 20, md: 24 }}>
          {stats.map((stat, index) => (
            <Box
              key={index}
              p={32}
              style={{
                backgroundColor: computedColorScheme === 'dark' ? '#0f172a' : '#ffffff',
                borderRadius: '16px',
                border:
                  computedColorScheme === 'dark'
            ...
            >
              <Text
                ...
                fw={900}
                c="#14b8a6"
                style={{ lineHeight: 1, marginBottom: '12px' }}
              >
                {stat.number}
              </Text>
